...
import "./FullMath.sol";
import "./ManaclesLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library ManaclesLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = ManaclesLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "ManaclesArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = ManaclesLibrary.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = ManaclesLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        IManaclesPair pair = IManaclesPair(
            ManaclesLibrary.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = IManaclesFactory(factory).feeTo() != address(0);
